...
function toSmartDate(timestamp) {
    ...
    if (t > 604800000) {
        ...
        s = Math.floor(t / 60000) + ' minute ago';
    }
    return s;
}
...
function _httpJSON(method, url, data, callback) {
    ...
    $.ajax(opt).done(function (r) {
        ...
        return callback({'error': 'http_bad_response', 'data': '' + jqXHR.status, 'message': 'Network error (HTTP ' + jqXHR.status + ')'});
    });
}
...
$(function() {
    if (location.pathname === '/' || location.pathname.indexOf('/shopping')===0) {
        $('li[data-url=shoppings]').addClass('uk-active');
    }
});
